...
// Vedanth completed the lq_notify function
int lq_notify(lqd_t _msgid, const struct sigevent *_notification) {
    // Check if the message queue descriptor is valid
    if (_msgid < 0) {
        errno = EBADF;
        return -1;
    }
    ...
    if (_notification->sigev_notify == SIGEV_NONE) {
        ...
        if (kill(sig.si_pid, sig.si_signo) == -1) {
            // handle error
            return -1;
        }
    } 
        ...
        _notification->sigev_notify_function(_notification->sigev_value);
    }
